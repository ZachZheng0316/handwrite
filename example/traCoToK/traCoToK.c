...
int main()
{
	...
	printf("please input num1 and num2(num1 <= num2):");
	scanf("%d %d", &num1, &num2);
	
	for(i = num1; i <= num2; i++) {
		sprintf(path1, "words/%d.txt", i);
		sprintf(path2, "traK/d%d.txt", i);
		tra_ink_to_sour(path1, coorfile);
		tra_sour_to_coor(coorfile, tra_coor);
		tra_coor_to_spe(tra_coor, tra_spe);
		tra_coor_to_posk(tra_coor, tra_spe, path2);
	}
